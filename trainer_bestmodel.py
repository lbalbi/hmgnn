...
class Train_BestModel:
    def __init__(self, model, epochs, train_loader, val_loader, full_cvgraph, full_graph,
        e_type, log, device, task, lr=0.001, mcfg=None, cfg=None, pstatement_sampler=False,
        nstatement_sampler=False, rstatement_sampler=False, contrastive_weight=0.1, gda_negs=None,
        state_list=None, no_contrastive=False, val_edges: torch.Tensor = None, val_edge_batch_size: int = None,
        protein_contrastive: bool = False, protein_k_neg: int = 5, protein_temperature: float = 0.5):
        ...
        if not self.no_contrastive:
            if self.protein_contrastive:
                if ProteinStatementSampler is None or ProteinContrastiveLoss is None:
                    raise ImportError(
                        "protein_contrastive=True but ProteinStatementSampler / ProteinContrastiveLoss "
                        "could not be imported.")
                if self.model.__class__.__name__ in {"GCN", "GAT", "GCN_GAE"}:
                    raise ValueError(
                        "protein_contrastive=True is not supported for homogeneous GCN/GAT/GCN_GAE models.")

                if self.nstatement_sampler:
                    # neg_statement edges removed from graph; use external edges from state_list
                    ...
                    # pos_statement edges removed from graph; use external edges from state_list
                    self.protein_sampler = PartialProteinSampler(
                        anchor_etype=self.e_type,
                        pos_etype="pos_statement",
                        neg_etype="neg_statement",
                    ...
                    self.protein_sampler = ProteinStatementSampler(anchor_etype=self.e_type, pos_etype="pos_statement",
                        neg_etype="neg_statement",go_etype="link", k_neg=self.protein_k_neg,
                        use_go_expansion_for_neg=True, seed=42)
                self.protein_sampler.prepare_global(self.full_cvgraph)
        ...
        self.alpha = float(contrastive_weight)
        ...
        for batch in self.train_loader:
            ...
            if (not self.no_contrastive) and self.protein_contrastive:
                ...
                if isinstance(self.protein_sampler, PartialProteinSampler):
                    anchors = torch.unique(pos_edge_index).tolist()
                    self.protein_sampler.prepare_batch(batch, anchor_nodes=anchors)
                else: self.protein_sampler.prepare_batch(batch)

            # neg_src, neg_dst = self.neg_sampler.sample(pos_edge_index.size(1))
            neg_src, neg_dst = self.neg_sampler.sample(pos_edge_index.size(1), src_override=pos_edge_index[0])
            ...
            if use_contrastive:
                if self.protein_contrastive: loss_contrast = self.contrastive(z_pos, z_neg)
                    ...
                    args = ((z_single, neg_statement_index) if self.model.__class__.__name__ in {"GCN", "GAT", "GCN_GAE"}
                        else (z_pos, z_neg, neg_statement_index))
...
class Test_BestModel:
    def __init__(self, model,test_loader, e_type, test_graph, full_graph, log, device,
        task, gda_negs=None, test_edges: torch.Tensor = None, test_edge_batch_size: int = None):
        self.model = model.to(device)
    ...
    def run(self):
        ...
        print(f"Accuracy: {acc:.4f}, F1 Score (W): {f1:.4f}, Precision (+): {precision_p:.4f}, "
            f"Recall (+): {recall_p:.4f}, Precision (-): {precision_n:.4f}, "
            f"Recall (-): {recall_n:.4f}, Roc Auc: {roc_auc:.4f}", flush=True)
        self.log.log("Test, final,"+str(acc)+","+str(f1)+","+str(precision_p)+","
            +str(recall_p)+","+str(precision_n)+","+str(recall_n)+","+str(roc_auc))
        self.log.close()
