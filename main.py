...
def main():
    ...
    parser.add_argument("--use_pstatement_sampler", action="store_true",
                        help="Use positive statement sampler to remove positives "
                             "from graph but keep in sampling")
    parser.add_argument("--use_nstatement_sampler", action="store_true",
                        help="Use negative statement sampler to remove negatives "
                             "from graph but keep in sampling")
    ...
    if ppi_key is None:
        raise ValueError(f"Could not find relation '{ppi_rel}' in loaded graph edge_types: {full_graph.edge_types}")

    ppi_ei = full_graph[ppi_key].edge_index
    all_eids = torch.arange(ppi_ei.size(1))
    explicit_test_ppis = dl.get_test_pairs(ppi_rel)

    split_helper_train = Pygloader(full_graph, ppi_rel=ppi_rel, batch_size=args.batch_size,
        val_split=0.0, device=device)

    split_id = args.path.split("/")[0]
    split_dir = os.path.join(split_id, f"splits_{args.task}")
    ...
    if explicit_test_ppis.numel() > 0:
        ...
        sampler_full_graph[ppi_key].edge_index = torch.cat([sampler_full_graph[ppi_key].edge_index,
                explicit_test_ppis.to(sampler_full_graph[ppi_key].edge_index.device)], dim=1)
    else:
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(trainval_eids), 1):
        ...
        train_loader = Pygloader(fold_train_graph, ppi_rel=ppi_rel, val_split=0.0,
            batch_size=args.batch_size, device=device)
        val_loader = Pygloader(fold_val_graph, ppi_rel=ppi_rel, val_split=0.0,
            batch_size=args.batch_size, device=device)
        ...
        log = Logger(f"{ModelCls.__name__ if args.model != 'gae' else 'GAE'}_fold{fold}",
            dir=args.output_dir)

        gda_negs = (dl.get_negative_edges() if hasattr(dl, "get_negative_edges")
            and (args.path == "gda_data" or args.path == "dp_data") else None)

        trainer = Train(model, args.CV_epochs, train_loader, val_loader,
            sampler_full_graph, fold_train_graph, e_type=ppi_rel, log=log, device=device,
            task=args.task, lrs=cfg["lr"], gda_negs=gda_negs, pstatement_sampler=args.use_pstatement_sampler,
            nstatement_sampler=args.use_nstatement_sampler, rstatement_sampler=args.use_rstatement_sampler,
            contrastive_weight=cfg.get("contrastive_weight", 0.1), state_list=state_list, no_contrastive=args.no_contrastive,
            val_edges=ppi_vei, val_edge_batch_size=args.batch_size)
    ...
    final_trainer = Train_BestModel(final_model, best_epoch, trainval_loader,
        [], full_cvgraph=trainval_graph, full_graph=sampler_full_graph, e_type=ppi_rel,
        log=final_log, device=device, task=args.task, lr=best_lr, contrastive_weight=best_alpha,
        state_list=state_list, pstatement_sampler=args.use_pstatement_sampler,
        nstatement_sampler=args.use_nstatement_sampler, rstatement_sampler=args.use_rstatement_sampler,
    ...
    tester = Test_BestModel(final_model, test_loader=test_loader, e_type=ppi_rel,
        test_graph=test_graph, full_graph=sampler_full_graph, log=final_log_test,
        device=device, task=args.task, gda_negs=gda_negs, test_edges=test_ppis,
