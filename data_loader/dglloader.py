...
class Dglloader:
    """
    Splits a DGLHeteroGraph into train/validation subsets based on the source nodes
    of a given edge type (e.g. "PPI") and yields mini-batch subgraphs by PPI edge IDs.

    Note: this loader does *not* control which edges are in the graph for testing.
    That is handled in `main` by constructing the appropriate subgraphs
    (e.g. train+val PPI edges only for train/val/test message passing).
    """
    def __init__(
        self,
        graph: dgl.DGLGraph,
    ...
    def _create_split_graph(self, ppi_eids: torch.Tensor) -> dgl.DGLGraph:
        ...
        Preserves original node IDs (relabel_nodes=False), so we can safely use global node indices.
        """
        eid_dict = {}
    ...
    def get_split_graphs(self) -> Tuple[dgl.DGLGraph, dgl.DGLGraph]:
