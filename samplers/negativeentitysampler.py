...
class NegativeEntitySampler(NegativeInstanceSampler):
    """Negative entity sampler using only the primary predicate (instance_rel).

    Pool construction uses ontological expansion via subclass_of:
      pos_pos:  positive neighbors to superclasses of anchor positives
      neg_neg:  negative neighbors to subclasses of anchor negatives
      neg_pos:  negative neighbors to subclasses of anchor positives
      pos_neg:  positive neighbors to superclasses of anchor negatives
    """
    ...
    def _build_pools_for_anchor(self, u: int) -> None:
        ...
        # For all predicates, build pos_pos from direct instance-instance or instance-class assertions.
        for pred in preds_u:
            pos_cls = self._pos_by_pred[u].get(pred, [])
            if not pos_cls:
                continue
            pos_sup = self._expand_up(pos_cls, self._subclass_successors)
            pools = _ensure_pool(self.pool_shared_pos_by_pred, u)
            pools[pred] = self._cap_pool(self._collect_instances(
                pos_sup, self._class2pos_by_pred.get(pred, {}), exclude=u
            ))
        ...
        # For neg pools, use per-relation pools from direct assertions only.
        for pred in preds_u:
            pos_cls = self._pos_by_pred[u].get(pred, [])
            neg_cls = self._neg_by_pred_direct[u].get(pred, [])

            if neg_cls:
                neg_sub = self._expand_negs(neg_cls, self._subclass_predecessors)
                pools = _ensure_pool(self.pool_shared_neg_by_pred, u)
                pools[pred] = self._cap_pool(self._collect_instances(
                    neg_sub, self._class2neg_by_pred.get(pred, {}), exclude=u
                ))
                pools = _ensure_pool(self.pool_pos_to_u_neg_by_pred, u)
                pools[pred] = self._cap_pool(self._collect_instances(
                    neg_sub, self._class2pos_by_pred.get(pred, {}), exclude=u
                ))

            if pos_cls:
                pos_sub = self._expand_negs(pos_cls, self._subclass_predecessors)
                pools = _ensure_pool(self.pool_neg_to_u_pos_by_pred, u)
                pools[pred] = self._cap_pool(self._collect_instances(
                    pos_sub, self._class2neg_by_pred.get(pred, {}), exclude=u
                ))

        # Track predicates considered.
        preds_tracked = set()
        pools_pos = self.pool_shared_pos_by_pred[u] or {}
        pools_sneg = self.pool_shared_neg_by_pred[u] or {}
        pools_pun = self.pool_pos_to_u_neg_by_pred[u] or {}
        pools_nup = self.pool_neg_to_u_pos_by_pred[u] or {}
        for pred in preds_u:
            if pools_pos.get(pred, empty).numel() > 0:
                preds_tracked.add(pred)
            if (pools_sneg.get(pred, empty).numel() > 0 or
                pools_pun.get(pred, empty).numel() > 0 or
                pools_nup.get(pred, empty).numel() > 0):
                preds_tracked.add(pred)
        primary = str(self.primary_pred)
        if primary not in preds_tracked and pools_pos.get(primary, empty).numel() > 0:
            preds_tracked.add(primary)
        if preds_tracked:
            self.preds_seen[u] = preds_tracked

        # Fallback: if either positive or negative neighbors are missing, build pools across relations.
        pos_any = any(t.numel() > 0 for t in pools_pos.values()) or \
            any(t.numel() > 0 for t in pools_sneg.values())
        neg_any = any(t.numel() > 0 for t in pools_pun.values()) or \
            any(t.numel() > 0 for t in pools_nup.values())
        if not pos_any or not neg_any:
            pos_cls_all = []
            neg_cls_all = []
            ...
            if not pos_any and pos_cls_all:
                pools = _ensure_pool(self.pool_shared_pos_by_pred, u)
    ...
    def prepare_global(self, full_g: HeteroData) -> None:
        ...
        if len(full_g.node_types) != 1:
            raise ValueError("NegativeEntitySampler assumes a single node type.")
