...
class NegativeInstanceSampler_NEWER:
    ...
    def _load_cache(self) -> bool:
        ...
        self._cur_stamp = 0
        print(f"[NegativeInstanceSampler_NEWER] loaded cache: {path}", flush=True)
        return True
    ...
    def _save_cache(self) -> None:
        ...
        print(f"[NegativeInstanceSampler_NEWER] saved cache: {path}", flush=True)
    ...
    def prepare_global(self, full_g: HeteroData) -> None:
        ...
        self.num_nodes = N
        print(f"[NegativeInstanceSampler_NEWER] prepare_global: start (num_nodes={N})")
        ...
        # print(f"[NegativeInstanceSampler_NEWER] prepare_global: instance_nodes={len(instance_nodes)}")
        ...
        # print(f"[NegativeInstanceSampler_NEWER] prepare_global: subclass_predecessors={len(subclass_predecessors)}")
        ...
        # print(f"[NegativeInstanceSampler_NEWER] prepare_global: edge scan took {time.time() - t_scan:.2f}s")
        ...
        # print(f"[NegativeInstanceSampler_NEWER] prepare_global: neg expansion took {time.time() - t_expand:.2f}s")
        ...
        # print(f"[NegativeInstanceSampler_NEWER] prepare_global: pool build took {time.time() - t_pools:.2f}s")
        self.anchors_valid = [u for u in self.anchors if self._has_any_pool(u)]
        self._g2l_cpu = torch.empty(self.num_nodes, dtype=torch.long)
        self._stamp_cpu = torch.zeros(self.num_nodes, dtype=torch.int32)
        self._cur_stamp = 0
        # print(f"[NegativeInstanceSampler_NEWER] prepare_global: done in {time.time() - t0:.2f}s")
        self._save_cache()
        self._pre_ready = False
    ...
    def prepare_epoch(self) -> None:
        ...
        # print(f"[NegativeInstanceSampler_NEWER] prepare_epoch: pre-sampled in {time.time() - t0:.2f}s", flush=True)
        ...
        if self.anchors:
            ...
            # print(
            #     "[NegativeInstanceSampler_NEWER] anchors without pool neighbors (global pools): "
    ...
    def get_contrastive_samples(self, z: Tensor, anchor_nodes: Optional[Tensor] = None,
        ...
        if n_id is None:
            ...
            if self._dbg_ctr % self._dbg_every == 0:
                ...
                # print(
                #     "[NegativeInstanceSampler_NEWER] get_contrastive_samples (full): "
        ...
        if self._dbg_ctr % self._dbg_every == 0:
            total = time.time() - t_start
            # print(
            #     "[NegativeInstanceSampler_NEWER] get_contrastive_samples (neighbor): "
            #     f"select={t_select:.3f}s sample={t_sample:.3f}s concat={t_concat:.3f}s total={total:.3f}s "
