...
class PartialStatementSampler:
    """
    PartialStatementSampler that supports two modes:
      --use_nstatement_sampler:
        - pos_etype="pos_statement", neg_etype="neg_statement"
        - 'neg_statement' edges are removed from the graph and passed in `neg_edges`
        - positives: pos_statement (graph)
        - negatives: neg_statement (state_list)
      --use_pstatement_sampler:
        - pos_etype="neg_statement", neg_etype="pos_statement"
        - 'pos_statement' edges are removed from the graph and passed in `neg_edges`
        - positives: pos_statement (state_list)
        - negatives: neg_statement (graph)
    """

    def __init__(self, k: int = 1, go_etype: str = "link",
        neg_edges: Optional[List[Tuple[int, int]]] = None):
        self.k = k
        self.go_etype = go_etype
        self.state_edges: List[Tuple[int, int]] = neg_edges or []
    ...
    @staticmethod
    def _find_rel_key(g: HeteroData, rel: str):
        """Return the edge key (src_nt, rel, dst_nt) for a given relation name, or None if absent."""
    ...
    def prepare_global(self, full_g: HeteroData, pos_etype: str = "pos_statement",
        ...
        """
        Args:
          full_g: full HeteroData graph *after* DataLoader has optionally removed
                  one of the statement relations.
          pos_etype: statement type that is still in the graph for this run.
          neg_etype: statement type that was removed from the graph and passed in `state_edges`.
        """

        present_rel = pos_etype
        removed_rel = neg_etype
        present_key = self._find_rel_key(full_g, present_rel)
        removed_key = self._find_rel_key(full_g, removed_rel)
        go_key = self._find_rel_key(full_g, self.go_etype)

        node_key = present_key or removed_key or go_key
        if node_key is None:
            raise ValueError("PartialStatementSampler: could not infer node type "
                "(no GO or statement edges found in the graph).")
        node_type = node_key[0]
        ...
        if present_key is not None and "edge_index" in full_g[present_key]:
            src_t, dst_t = full_g[present_key].edge_index
            src_list = src_t.tolist()
            dst_list = dst_t.tolist()
            if present_rel == "pos_statement": pos_src, pos_dst = src_list, dst_list
            elif present_rel == "neg_statement": neg_src, neg_dst = src_list, dst_list

        if removed_key is not None and "edge_index" in full_g[removed_key] and not self.state_edges:
            src_t, dst_t = full_g[removed_key].edge_index
            src_list = src_t.tolist()
            dst_list = dst_t.tolist()
            if removed_rel == "pos_statement":  pos_src, pos_dst = src_list, dst_list
            elif removed_rel == "neg_statement":  neg_src, neg_dst = src_list, dst_list

        if self.state_edges:
            s_src, s_dst = zip(*self.state_edges)
            s_src = list(s_src)
            s_dst = list(s_dst)
            if removed_rel == "pos_statement": pos_src, pos_dst = s_src, s_dst
            elif removed_rel == "neg_statement": neg_src, neg_dst = s_src, s_dst

        pos_global: List[List[int]] = [[] for _ in range(N)]
        for u, v in zip(pos_src, pos_dst):
            if 0 <= u < N and 0 <= v < N: pos_global[u].append(v)
        self.pos_global = pos_global
        self.pos_targets = set(pos_dst)

        direct_global: List[List[int]] = [[] for _ in range(N)]
        for u, v in zip(neg_src, neg_dst):
            if 0 <= u < N and 0 <= v < N: direct_global[u].append(v)

        predecessors: Dict[int, List[int]] = {}
        if go_key is not None and "edge_index" in full_g[go_key]:
    ...
    def prepare_batch(self, batch: HeteroData):
        """
        Assumes that batch['node'].num_nodes == full.num_nodes (your Pygloader
        keeps all nodes in every batch), so global == local node IDs.
        """
        ...
        go_key = self._find_rel_key(batch, self.go_etype)
        if go_key is None: node_type = next(iter(batch.node_types))
        else: node_type = go_key[0]

        B = _num_nodes_of(batch, node_type)
        self.orig = torch.arange(B, dtype=torch.long)
        self.mapping = {int(g.item()): int(i) for i, g in enumerate(self.orig)}
    ...
    def sample(self) -> Tensor:
        """
        Vectorized sampling of k negatives per node
        """
    ...
    def get_contrastive_samples(self, z: Tensor, neg_ei: Tensor) -> Tuple[Tensor, Tensor, Tensor]:
        ...
        Returns:
            z_pos:     (B, D)  embeddings of each node (anchor)
        ...
        z_pos = z
        pos_nb: List[int] = []
        orig_list = self.orig.tolist() if self.orig is not None else list(range(B))
        for i, g in enumerate(orig_list):
            opts = self.pos_global[g] if g < len(self.pos_global) else []
            if opts:
                g2 = random.choice(opts)
                pos_nb.append(self.mapping.get(int(g2), i))
