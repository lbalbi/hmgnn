...
class NegativeInstanceSampler_V2:
    ...
    def __init__(self, k: int = 2, subclass_rel: str = "subclass_of",
        ...
        self._dbg_ctr = 0
        self._dbg_every = 20
        # Hard-negative sampling (low-cost): pick top-k within a small random candidate set.
        self.hard_negatives = True
        self.hard_neg_mult = 5
        self.hard_neg_min_candidates = 8
        self.hard_neg_max_candidates = 15
        self.hard_neg_refresh = 2
        self._epoch_counter = 0
        self._pre_cand_pos2neg = None
        self._pre_cand_neg2pos = None
        self.hard_neg_cache_batches = 4
        self.max_contrastive_anchors = 3072
        self._batch_counter = 0
        self._cache_full = {}
        self._cache_neighbor = {}

    def print_pool_stats(self, prefix: str = "[NegativeInstanceSampler_V2]") -> None:
        if not self.anchors:
    ...
    def _load_cache(self) -> bool:
        ...
        print(f"[NegativeInstanceSampler_V2] loaded cache: {path}", flush=True)
        return True
    ...
    def _save_cache(self) -> None:
        ...
        print(f"[NegativeInstanceSampler_V2] saved cache: {path}", flush=True)
    ...
    def _hard_candidate_k(self) -> int:
        k = int(self.k)
        return max(self.hard_neg_min_candidates, min(self.hard_neg_max_candidates, k * self.hard_neg_mult))

    def _select_hard_negatives(self, anchor_idx: int, cand_idx: Tensor, z: Tensor) -> Tensor:
        """Pick top-k cosine-similar candidates to anchor from cand_idx."""
        k = int(self.k)
        device = z.device
        if cand_idx.numel() == 0:
            return torch.full((k,), int(anchor_idx), dtype=torch.long, device=device)
        cand = cand_idx.to(device)
        if cand.dim() == 0:
            cand = cand.view(1)
        cand = torch.unique(cand)
        cand = cand[cand != int(anchor_idx)]
        if cand.numel() == 0:
            return torch.full((k,), int(anchor_idx), dtype=torch.long, device=device)
        with torch.no_grad():
            a = z[int(anchor_idx)]
            a = F.normalize(a, dim=0)
            c = F.normalize(z[cand], dim=1)
            sim = (c * a).sum(dim=1)
            topk = torch.topk(sim, k=min(k, int(cand.numel())), largest=True).indices
            chosen = cand[topk]
        if chosen.numel() < k:
            pad = torch.full((k - int(chosen.numel()),), int(anchor_idx), dtype=torch.long, device=device)
            chosen = torch.cat([chosen, pad], dim=0)
        return chosen.long()

    def _select_hard_negatives_batch(self, anchor_idx: Tensor, cand_idx: Tensor, z_norm: Tensor) -> Tensor:
        """Vectorized hard-negative selection.
        anchor_idx: (B,) long, cand_idx: (B, C) long, z_norm: (N, D) normalized.
        Returns (B, k) long indices.
        """
        if anchor_idx.numel() == 0:
            return torch.empty((0, int(self.k)), dtype=torch.long, device=z_norm.device)
        B, C = cand_idx.shape
        if C == 0:
            return anchor_idx.view(-1, 1).repeat(1, int(self.k))
        a = z_norm[anchor_idx]                      # (B, D)
        c = z_norm[cand_idx]                        # (B, C, D)
        sim = (c * a.unsqueeze(1)).sum(dim=-1)      # (B, C)
        k = min(int(self.k), C)
        topk = torch.topk(sim, k=k, dim=1).indices  # (B, k)
        chosen = torch.gather(cand_idx, 1, topk)    # (B, k)
        if k < int(self.k):
            pad = anchor_idx.view(-1, 1).repeat(1, int(self.k) - k)
    ...
    def prepare_global(self, full_g: HeteroData) -> None:
        ...
        self.num_nodes = N
        print(f"[NegativeInstanceSampler_V2] prepare_global: start (num_nodes={N})")
        ...
        # print(f"[NegativeInstanceSampler_V2] prepare_global: instance_nodes={len(instance_nodes)}")
        ...
        # print(f"[NegativeInstanceSampler_V2] prepare_global: subclass_predecessors={len(subclass_predecessors)}")
        ...
        # print(f"[NegativeInstanceSampler_V2] prepare_global: edge scan took {time.time() - t_scan:.2f}s")
        ...
        # print(f"[NegativeInstanceSampler_V2] prepare_global: neg expansion took {time.time() - t_expand:.2f}s")
        ...
        # print(f"[NegativeInstanceSampler_V2] prepare_global: pool build took {time.time() - t_pools:.2f}s")
        self.anchors_valid = [u for u in self.anchors if self._has_any_pool(u)]
        self._g2l_cpu = torch.empty(self.num_nodes, dtype=torch.long)
        self._stamp_cpu = torch.zeros(self.num_nodes, dtype=torch.int32)
        self._cur_stamp = 0
        # print(f"[NegativeInstanceSampler_V2] prepare_global: done in {time.time() - t0:.2f}s")
        self._save_cache()
        self._compute_prefer_inst_only()
        self._pre_ready = False
    ...
    def prepare_epoch(self) -> None:
        ...
        refresh_hard = self.hard_negatives and (
            self._pre_cand_pos2neg is None
            or self._pre_cand_neg2pos is None
            or (self.hard_neg_refresh > 0 and (self._epoch_counter % self.hard_neg_refresh == 0))
        )
        cand_k = self._hard_candidate_k()
        if refresh_hard:
            cand_base = torch.arange(N, dtype=torch.long).unsqueeze(1).repeat(1, cand_k)
            self._pre_cand_pos2neg = cand_base.clone()
            self._pre_cand_neg2pos = cand_base.clone()
        ...
        for u in self.anchors_valid:
            ...
            if not self.hard_negatives:
                ...
                self._pre_cand_pos2neg[u] = self._sample_k_priority_cpu(
                    pos2neg_pool, pred_order, stamp=0,
                    fallback_global=u, fallback_local=u, k=cand_k, full_graph_mode=True
                )
                self._pre_cand_neg2pos[u] = self._sample_k_priority_cpu(
                    neg2pos_pool, pred_order, stamp=0,
                    fallback_global=u, fallback_local=u, k=cand_k, full_graph_mode=True
                )
            if has_shpos:
        ...
        # print(f"[NegativeInstanceSampler_V2] prepare_epoch: pre-sampled in {time.time() - t0:.2f}s", flush=True)
        ...
        if self.anchors:
            ...
            # print(
            #     "[NegativeInstanceSampler_V2] anchors without pool neighbors (global pools): "
    ...
    def get_contrastive_samples(self, z: Tensor, anchor_nodes: Optional[Tensor] = None,
        ...
        if n_id is None:
            ...
            cand_pos2neg_cpu, cand_neg2pos_cpu = [], []
            z_norm = F.normalize(z, dim=1) if self.hard_negatives else None
            ...
            # Subsample anchors before building per-anchor candidate pools
            if self.max_contrastive_anchors and anchor_globals_cpu.numel() > self.max_contrastive_anchors:
            ...
            for u in anchor_globals_cpu.tolist():
                ...
                if has_shneg:
                    shneg_cpu.append(self._sample_k_priority_cpu(
                        shneg_pool, pred_order, stamp=0,
                        fallback_global=u, fallback_local=u, k=k, full_graph_mode=True
                    ).unsqueeze(0))
                if self.hard_negatives:
                    cand_pos2neg = None
                    cand_neg2pos = None
                    if self._pre_cand_pos2neg is not None and u < self._pre_cand_pos2neg.size(0):
                        cand_pos2neg = self._pre_cand_pos2neg[u]
                    if self._pre_cand_neg2pos is not None and u < self._pre_cand_neg2pos.size(0):
                        cand_neg2pos = self._pre_cand_neg2pos[u]
                    if cand_pos2neg is None:
                        cand_k = self._hard_candidate_k()
                        cand_pos2neg = self._sample_k_priority_cpu(
                            pos2neg_pool, pred_order, stamp=0,
                            fallback_global=u, fallback_local=u, k=cand_k, full_graph_mode=True
                        )
                    if cand_neg2pos is None:
                        cand_k = self._hard_candidate_k()
                        cand_neg2pos = self._sample_k_priority_cpu(
                            neg2pos_pool, pred_order, stamp=0,
                            fallback_global=u, fallback_local=u, k=cand_k, full_graph_mode=True
                        )
                    cand_pos2neg_cpu.append(cand_pos2neg.unsqueeze(0))
                    cand_neg2pos_cpu.append(cand_neg2pos.unsqueeze(0))
                else:
                    pos2neg_cpu.append(self._sample_k_priority_cpu(
                        pos2neg_pool, pred_order, stamp=0,
                        fallback_global=u, fallback_local=u, k=k, full_graph_mode=True
                    ).unsqueeze(0))
                    neg2pos_cpu.append(self._sample_k_priority_cpu(
                        neg2pos_pool, pred_order, stamp=0,
                        fallback_global=u, fallback_local=u, k=k, full_graph_mode=True
                    ).unsqueeze(0))
                if has_shpos:
                    shpos_cpu.append(self._sample_k_priority_cpu(
                        shpos_pool, pred_order, stamp=0,
                        fallback_global=u, fallback_local=u, k=k, full_graph_mode=True
                    ).unsqueeze(0))
            ...
            if self.hard_negatives:
                use_cache = (self.hard_neg_cache_batches > 0
                    and (self._batch_counter % self.hard_neg_cache_batches != 0)
                    and bool(self._cache_full))
                if use_cache:
                    pos2neg = self._cache_full["pos2neg"]
                    neg2pos = self._cache_full["neg2pos"]
                else:
                    cand_pos2neg = torch.cat(cand_pos2neg_cpu, dim=0).to(device)
                    cand_neg2pos = torch.cat(cand_neg2pos_cpu, dim=0).to(device)
                    pos2neg = self._select_hard_negatives_batch(anchors, cand_pos2neg, z_norm)
                    neg2pos = self._select_hard_negatives_batch(anchors, cand_neg2pos, z_norm)
                    self._cache_full = {"pos2neg": pos2neg.detach(), "neg2pos": neg2pos.detach()}
            else:
            ...
            if self._dbg_ctr % self._dbg_every == 0:
                ...
                # print(
                #     "[NegativeInstanceSampler_V2] get_contrastive_samples (full): "
        ...
        if anchor_globals_list:
            if self.max_contrastive_anchors and len(anchor_globals_list) > self.max_contrastive_anchors:
                perm = torch.randperm(len(anchor_globals_list))[: self.max_contrastive_anchors]
                anchor_globals_list = [anchor_globals_list[i] for i in perm.tolist()]
                anchor_locals_list = [anchor_locals_list[i] for i in perm.tolist()]
            ...
            if self._pre_ready and self._pre_shneg is not None and not self.hard_negatives:
                ...
                for u_g, u_l in zip(anchor_globals_list, anchor_locals_list):
                    ...
                    if has_shneg:
                        shneg_cpu.append(self._sample_k_priority_cpu(
                            shneg_pool, pred_order, stamp=stamp,
                            fallback_global=u_g, fallback_local=u_l, k=k, full_graph_mode=False
                        ).unsqueeze(0))
                    if self.hard_negatives:
                        cand_k = self._hard_candidate_k()
                        cand_pos_local = self._sample_k_priority_cpu(
                            pos2neg_pool, pred_order, stamp=stamp,
                            fallback_global=u_g, fallback_local=u_l, k=cand_k, full_graph_mode=False
                        )
                        cand_neg_local = self._sample_k_priority_cpu(
                            neg2pos_pool, pred_order, stamp=stamp,
                            fallback_global=u_g, fallback_local=u_l, k=cand_k, full_graph_mode=False
                        )
                        cand_pos2neg_cpu.append(cand_pos_local.unsqueeze(0))
                        cand_neg2pos_cpu.append(cand_neg_local.unsqueeze(0))
                    else:
                        pos2neg_cpu.append(self._sample_k_priority_cpu(
                            pos2neg_pool, pred_order, stamp=stamp,
                            fallback_global=u_g, fallback_local=u_l, k=k, full_graph_mode=False
                    ...
                    if has_shpos:
                        shpos_cpu.append(self._sample_k_priority_cpu(
                            shpos_pool, pred_order, stamp=stamp,
                            fallback_global=u_g, fallback_local=u_l, k=k, full_graph_mode=False
                        ).unsqueeze(0))
        t_sample = time.time() - t_sample
        ...
        shpos = torch.cat(shpos_cpu, dim=0).to(device)
        if self.hard_negatives:
            # Disable caching in neighbor mode to avoid stale local indices.
            cand_pos2neg = torch.cat(cand_pos2neg_cpu, dim=0).to(device)
            cand_neg2pos = torch.cat(cand_neg2pos_cpu, dim=0).to(device)
            pos2neg = self._select_hard_negatives_batch(anchors_local, cand_pos2neg, z_norm)
            neg2pos = self._select_hard_negatives_batch(anchors_local, cand_neg2pos, z_norm)
        else:
            pos2neg = torch.cat(pos2neg_cpu, dim=0).to(device)
            neg2pos = torch.cat(neg2pos_cpu, dim=0).to(device)
        t_concat = time.time() - t_concat
        ...
        if self._dbg_ctr % self._dbg_every == 0:
            total = time.time() - t_start
            # print(
            #     "[NegativeInstanceSampler_V2] get_contrastive_samples (neighbor): "
            #     f"select={t_select:.3f}s sample={t_sample:.3f}s concat={t_concat:.3f}s total={total:.3f}s "
